...
jjm --locator "${REMOTE}" --build-command script/build.sh --junit 'target/surefire-reports/junitreports/TEST-*.xml' --checkstyle target/checkstyle-result.xml --recipients funtimecoding@gmail.com > job.xml
